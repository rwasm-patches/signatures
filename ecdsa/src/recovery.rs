    ...
    SignatureSize<C>: ArrayLength<u8>,
{
    ...
    /// the function will use [`crate::rwasm::VerifyingKey::recover_from_prehash_secp256k1`] to return a precomputed public key.
    ...
    pub fn recover_from_prehash(
        ...
        // Note: An additional check can be added to ensure the field bytes size is 32 bytes, but this is not necessary.
        #[cfg(feature = "hazmat")]
        if C::ORDER
            == <C as Curve>::Uint::decode_field_bytes(GenericArray::from_slice(&SECP256K1_ORDER))
            && prehash.len() == 32
        {
